...
(function () {
 ...
 // Product-detail current item + quantity.
 detailProduct: null,
 detailQty: 1,
 ...
 const BHAI_AGENT = { name: 'তালহা ভাই', initial: 'ত', role: 'প্রিয়শপ সহকারী', sla: 'সাধারণত ২ মিনিটে উত্তর' };
 ...
 if (fab) fab.style.display = (screenId === 'bhai' || screenId === 'product-detail' || state.simpleMode)? 'none' : 'flex';
 ...
 try {
 const g = re => { const x = payload.match(re); return x? x[1] : null; };
 const name = g(/name:'([^']+)'/);
 const price = g(/price:(\d+)/);
 const img = g(/image:'([^']+)'/);
 if (name && price && img) {
 const cardImg = el.querySelector('.product-image img');
 populateProductDetail({
 id: parseInt(g(/id:(\d+)/) || '0', 10),
 name,
 nameEn: g(/nameEn:'([^']+)'/) || name,
 weight: g(/weight:'([^']+)'/) || '',
 price: parseInt(price, 10),
 image: img,
 imgFilter: g(/imgFilter:'([^']+)'/) || (cardImg? cardImg.style.filter : '')
 });
 }
 } catch (e) {
 ...
 <img loading="lazy" src="${icon}" style="width:100%;height:100%;object-fit:cover;" alt="product">
 </div>`;
 }
 ...
 <img loading="lazy" src="${item.image || './images/rice_akij.png'}" style="width:100%;height:100%;object-fit:cover;filter:${item.imgFilter || ''}" alt="">
 </div>
...
? `<div class="cart-summary-row" style="color:var(--success)"><span>ক্যাশব্যাক ভাউচার</span><span>-৳${toBn(voucher.toLocaleString())}</span></div>`
 ...
 setText('detail-qty', toBn(state.detailQty));
 }

 function detailAddToCart() {
 const p = state.detailProduct;
 if (!p) { navigateTo('cart'); return; }
 const ex = state.cartItems.find(i => i.id === p.id);
 if (ex) ex.qty += state.detailQty;
 else state.cartItems.push({ id: p.id, name: p.name, nameEn: p.nameEn, weight: p.weight, price: p.price, qty: state.detailQty, image: p.image, imgFilter: p.imgFilter });
 renderCart();
 ...
 state.detailProduct = d;
 state.detailQty = 1;
 setText('detail-qty', toBn(1));
...
? `<button class="reorder-swap-btn" onclick="event.stopPropagation();App.swapAlternative(${item.id})"><i data-lucide="repeat" style="width:13px;height:13px;pointer-events:none"></i> বিকল্প নিন (${item.alt.name})</button>`
 ...
 <div class="cart-item-image" style="background:#f5f5f5;overflow:hidden;">
 <img loading="lazy" src="${item.image}" style="width:100%;height:100%;object-fit:cover;opacity:.5;filter:${item.imgFilter || ''}" alt="">
 </div>
 ...
 <div class="cart-item-image" style="background:#f5f5f5;overflow:hidden;">
 <img loading="lazy" src="${item.image}" style="width:100%;height:100%;object-fit:cover;filter:${item.imgFilter || ''}" alt="">
 </div>
 <div class="reorder-item-details">
 <div class="cart-item-name">${item.name}</div>
...
? voiceUnrecognizedHtml(it, i)
 ...
 <div class="voice-sugg-img"><img loading="lazy" src="${s.image}" style="width:100%;height:100%;object-fit:cover;filter:${s.imgFilter || ''}" alt=""></div>
...
? '<span class="voice-badge match"><i data-lucide="check" style="width:12px;height:12px"></i> মিল পাওয়া গেছে</span>'
 ...
 <div class="cart-item-image" style="background:#f5f5f5;overflow:hidden;"><img loading="lazy" src="${p.image}" style="width:100%;height:100%;object-fit:cover;filter:${p.imgFilter || ''}" alt=""></div>
